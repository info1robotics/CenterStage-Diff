...
import org.firstinspires.ftc.teamcode.roadrunner.drive.BulkStandardTrackingWheelLocalizer;
...
public class TrackingWheelLateralDistanceTuner extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (!(drive.getLocalizer() instanceof BulkStandardTrackingWheelLocalizer)) {
            RobotLog.setGlobalErrorMsg("StandardTrackingWheelLocalizer is not being set in the "
                    + "drive class. Ensure that \"setLocalizer(new StandardTrackingWheelLocalizer"
                    + "(hardwareMap));\" is called in SampleMecanumDrive.java");
        }

        log.add("Prior to beginning the routine, please read the directions located in the comments of the opmode file.")
                .add("Press play to begin the tuning routine.")
                .add("")
                .add("Press Y/△ to stop the routine.")
                .tick();
        ...
        while (!isStopRequested() && !tuningFinished) {
            ...
            log.clear()
                    ...
                    .add("Total Heading (deg): " + Math.toDegrees(headingAccumulator))
                    .add("Raw Heading (deg): " + Math.toDegrees(heading))
                    .add("")
                    .add("Press Y/△ to conclude routine")
                    .tick();
        ...
        log.clear()
                .add("Localizer's total heading: " + Math.toDegrees(headingAccumulator) + "°")
                .add("Effective LATERAL_DISTANCE: " + (headingAccumulator / (NUM_TURNS * Math.PI * 2)) * BulkStandardTrackingWheelLocalizer.LATERAL_DISTANCE)
                .tick();
